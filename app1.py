...
def get_real_crack_prediction(image_bytes, _model):
    ...
    THRESH_HAZARDOUS_HIGH = 90
    THRESH_HAZARDOUS_LOW = 80
...
# --- [CORRECTED] CHATBOT FUNCTION (MODIFIED FOR GOOGLE GEMINI) ---
def get_chatbot_response(messages_history):
    """
    Calls Google Gemini using the provided message history.
    Assumes history is in: [{"role": "...", "parts": ["..."]}] format.
    """
    if not GOOGLE_API_KEY:
        return ("Error: Google API Key is not configured. "
                "Please add it to your `.streamlit/secrets.toml` file (as GOOGLE_API_KEY).")
        
    try:
        genai.configure(api_key=GOOGLE_API_KEY)
        
        model = genai.GenerativeModel(
            model_name="gemini-1.5-flash-latest", # <-- THIS MODEL WILL WORK AFTER YOU UPGRADE
            system_instruction=system_prompt 
        )
        
        # --- Conversion Layer ---
        # Convert the "assistant" role to "model" for the API
        gemini_history_for_api = []
        for msg in messages_history:
            # Use "model" role if sender was "assistant", otherwise use the role as-is
            role = "model" if msg["role"] == "assistant" else msg["role"]
            gemini_history_for_api.append({"role": role, "parts": msg["parts"]})
        
        # Generate the response
        response = model.generate_content(gemini_history_for_api)
        return response.text
    
    except Exception as e:
        if "API key not valid" in str(e):
            return "Error: The Google API Key in your `secrets.toml` file is incorrect or invalid."
        if "quota" in str(e):
            return "Error: The Google API account has exceeded its quota. Please check your billing."
        # This will show you the "v1beta" error until you upgrade
        return f"*Error: Could not connect to Gemini AI.\n\nDetails:* {e}"
# --- END OF FIX ---
...
# --- [CORRECTED] Initialize Chat History (MODIFIED FOR GEMINI) ---
if "messages" not in st.session_state:
    st.session_state.messages = [
        # Use "assistant" for consistency in st.session_state
        # The chatbot function will convert this to "model" for the API
        {"role": "assistant", "parts": ["Welcome! I am SCAN ASSISTANT. Ask me how to use this tool or about the implications of cracks in roads and buildings."]}
    ]
# --- END OF FIX ---
...
if uploaded_file is not None:
    ...
    if report["status"] == "DANGER":
        ...
        st.warning(f"### ⚠️ STATUS: {report['status']} (Severity > 60)")
        st.metric(label="Crack Severity Score", value=f"{report['score']:.1f} / 100")
        st.subheader("Recommended Action")
        st.warning(
    ...
    # --- [CORRECTED] AUTO-CHATBOT LOGIC (Now compatible with Gemini) ---
    if st.session_state.analysis_context and not st.session_state.get("auto_message_sent", False):
        ...
        api_messages = st.session_state.messages + [
            {"role": "user", "parts": [auto_prompt]}  # <-- USE "parts"
        ]
        ...
        st.session_state.messages.append({"role": "assistant", "parts": [response]}) # <-- USE "assistant" and "parts"
...
# --- Sidebar Information (FIXED URL) ---
st.sidebar.title(" SCAN-Structural Crack Analysis Network")
st.sidebar.image("https://i.imgur.com/qgA9YVj.png", width=150) # <-- FIXED URL
st.sidebar.info(
...
st.sidebar.markdown(
    ...
    * **LLM Assistant:** Google Gemini
    """
)
st.sidebar.markdown("---")


# --- [CORRECTED] CHATBOT INTERFACE ---
...
with st.container(height=350):
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            st.markdown(message["parts"][0]) # <-- READ FROM "parts"[0]
...
if prompt := st.chat_input("Ask about this tool or crack implications..."):
    ...
    st.session_state.messages.append({"role": "user", "parts": [prompt]}) # <-- USE "parts"
    ...
    if st.session_state.analysis_context:
        ...
        # Append this context as a user message for the API
        api_messages.append({"role": "user", "parts": [context_str]}) # <-- USE "user" and "parts"
